...
float get_order_cost(Order* order_ptr){
    float sum_cost = 0;
    for (int i=0; i < order_ptr->num_contents; i++) {
        sum_cost += order_ptr->contents[i].price;
    }
    return sum_cost;
}
...
void add_item_to_order(Order* order_ptr, char product_name[], float product_price ){
    int len = order_ptr->num_contents;
    order_ptr->contents[len].price = product_price;
    strcpy(order_ptr->contents[len].name, product_name);
